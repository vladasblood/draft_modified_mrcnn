...
class RepPointsHead(AnchorFreeHead):
    ...
    def _bbox_decode(self, points: Tensor, bbox_pred: Tensor, stride: int,
        ...
        Args:
            ...
            bbox_pred (Tensor): shape (N, 18) – 9 points × (dx, dy).
        ...
        """
        # Reshape to (N, 9, 2)
        pred_points = bbox_pred.view(-1, 9, 2)

        # Scale and offset to get absolute point coordinates
        pred_points = pred_points * stride + points.unsqueeze(1)  # (N, 9, 2)

        # Compute bounding box from min/max of predicted points
        x_min, _ = pred_points[:, :, 0].min(dim=1)
        y_min, _ = pred_points[:, :, 1].min(dim=1)
        x_max, _ = pred_points[:, :, 0].max(dim=1)
        y_max, _ = pred_points[:, :, 1].max(dim=1)
